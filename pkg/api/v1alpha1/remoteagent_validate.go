...
// Validate runs structural validation on the RemoteAgent envelope.
func (r *RemoteAgent) Validate() error {
	var errs FieldErrors
	errs = append(errs, ValidateObjectMeta(r.Metadata)...)
	errs = append(errs, validateRemoteAgentSpec(&r.Spec)...)
	if len(errs) == 0 {
		return nil
	}
	return errs
...
func validateRemoteAgentSpec(s *RemoteAgentSpec) FieldErrors {
	var errs FieldErrors
