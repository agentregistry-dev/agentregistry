-- Models: admin-owned deployable model definitions (provider-scoped identity
-- plus platform-owned auth/endpoint posture). A mutable-object kind keyed by
-- (namespace, name), like runtimes and deployments: auth/endpoint edits are
-- routine config mutations, not new versions. Deployments select one via
-- spec.modelRef. Wires the same updated-at, status-notify, and
-- control-plane-event triggers so Model edits wake the Deployment controller
-- and propagate to referencing deployments.
