...
func TestRegisterApply_MultiDocRoundTrip(t *testing.T) {
	...
	agents := v1alpha1store.NewStore(pool, "agents")
	mcps := v1alpha1store.NewStore(pool, "mcp_servers")
...
func TestRegisterApply_MutableObjectResultsDoNotExposeVersion(t *testing.T) {
	pool := v1alpha1store.NewTestPool(t)
	runtimes := v1alpha1store.NewMutableObjectStore(pool, "runtimes")
	deployments := v1alpha1store.NewMutableObjectStore(pool, "deployments")
...
func TestRegisterApply_DefaultsRemoteMCPServerTagBeforeAuthorize(t *testing.T) {
	pool := v1alpha1store.NewTestPool(t)
	mcpServers := v1alpha1store.NewStore(pool, "mcp_servers")
