...
func TestResourceRegister_AgentCRUD(t *testing.T) {
	...
	store := v1alpha1store.NewStore(pool, "agents")
...
func TestResourceRegister_MutableObjectUsesNameOnlyRoute(t *testing.T) {
	pool := v1alpha1store.NewTestPool(t)
	store := v1alpha1store.NewMutableObjectStore(pool, "runtimes")
...
func TestResourceRegister_ResolverDetectsDanglingRef(t *testing.T) {
	pool := v1alpha1store.NewTestPool(t)
	agentStore := v1alpha1store.NewStore(pool, "agents")
	mcpStore := v1alpha1store.NewStore(pool, "mcp_servers")
