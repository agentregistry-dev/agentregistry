...
func TestScanStatus(t *testing.T) {
	...
	// A key the format rules do not check must not fail the scan.
...
// TestTerminalStatus checks that a terminal failure records the scan version,
// clears the scan results, and keeps the resolved source.
