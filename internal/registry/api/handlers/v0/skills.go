...
func RegisterSkillsEndpoints(api huma.API, pathPrefix string, registry service.RegistryService) {
    ...
    huma.Register(api, huma.Operation{
        ...
        if input.Search != "" { filter.SubstringName = &input.Search }
        if input.Version != "" {
    ...
    huma.Register(api, huma.Operation{
        ...
        if err != nil { return nil, huma.Error400BadRequest("Invalid skill name encoding", err) }
        version, err := url.PathUnescape(input.Version)
        if err != nil { return nil, huma.Error400BadRequest("Invalid version encoding", err) }
    ...
    huma.Register(api, huma.Operation{
        ...
        skillValues := make([]skillmodels.SkillResponse, len(skills))
        for i, s := range skills { skillValues[i] = *s }
        return &Response[skillmodels.SkillListResponse]{
            Body: skillmodels.SkillListResponse{
                Skills: skillValues,
                Metadata: skillmodels.Metadata{ Count: len(skills) },
            },
        }, nil
    })
}
