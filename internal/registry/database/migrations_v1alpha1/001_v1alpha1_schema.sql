...
-- coexist with the legacy `public.agents`, `public.servers`, etc. during the
-- incremental migration. Callers that use the new generic Store qualify
-- table names as "v1alpha1.<table>"; legacy postgres_*.go stores continue
-- to read/write the unqualified public tables without conflict. The final
-- cutover (once every subsystem is ported off legacy) drops the old tables
-- and either keeps the v1alpha1 schema or renames it to public.
...
CREATE UNIQUE INDEX v1alpha1_agents_latest_version  ON v1alpha1.agents (name) WHERE is_latest_version;
...
CREATE UNIQUE INDEX v1alpha1_mcp_servers_latest_version  ON v1alpha1.mcp_servers (name) WHERE is_latest_version;
...
CREATE UNIQUE INDEX v1alpha1_skills_latest_version  ON v1alpha1.skills (name) WHERE is_latest_version;
...
CREATE UNIQUE INDEX v1alpha1_prompts_latest_version  ON v1alpha1.prompts (name) WHERE is_latest_version;
...
CREATE UNIQUE INDEX v1alpha1_providers_latest_version  ON v1alpha1.providers (name) WHERE is_latest_version;
...
CREATE UNIQUE INDEX v1alpha1_deployments_latest_version  ON v1alpha1.deployments (name) WHERE is_latest_version;
...
INSERT INTO v1alpha1.providers (name, version, spec, is_latest_version)
VALUES
