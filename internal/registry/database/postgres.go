...
func (db *PostgreSQL) ListSkills(ctx context.Context, tx pgx.Tx, filter *SkillFilter, cursor string, limit int) ([]*skillmodels.SkillResponse, string, error) {
    ...
    for rows.Next() {
        ...
        resp := &skillmodels.SkillResponse{
            Skill: skillJSON,
            Meta: skillmodels.ResponseMeta{
                Official: &skillmodels.RegistryExtensions{
                    Status:      status,
                    PublishedAt: publishedAt,
                    UpdatedAt:   updatedAt,
                    IsLatest:    isLatest,
...
func (db *PostgreSQL) CreateSkill(ctx context.Context, tx pgx.Tx, skillJSON *skillmodels.SkillJSON, officialMeta *skillmodels.RegistryExtensions) (*skillmodels.SkillResponse, error) {
    if ctx.Err() != nil {
        return nil, ctx.Err()
    }
