...
// claudePlugin finds a Claude plugin's components where Claude Code loads
// them: the default folders and files, plus the paths and inline parts that
// the manifest declares (code.claude.com/docs/en/plugins-reference).
...
// skillFileMatcher matches skills/<name>/SKILL.md, the skills in each
// directory the skills key adds, and a root SKILL.md when nothing else
// declares skills.
...
// mcpEntryServerNames lists the servers of one mcpServers entry: an inline
// map, a .json file, or an MCP bundle. A bundle is listed by its path or URL,
// because its server config sits inside the archive.
...
// mcpFileServerNames lists the servers in an MCP config file, which Claude
// Code reads with or without the mcpServers wrapper. An absent or malformed
// file has none.
