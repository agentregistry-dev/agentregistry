// Package format detects a plugin bundle's layout from its manifest files.
// Raise v1alpha1.PluginScanVersion with any change to these rules.
package format
...
// manifestKeys are the only manifest keys the rules check. Other keys are ignored.
...
// manifest holds the checked keys with the JSON types they require.
...
// manifestAuthor is the object form of author, the only form allowed.
...
// selectManifest picks the root plugin.json, else the Claude manifest.
// A manifest path that is a directory is rejected.
...
// decodeManifest decodes the checked keys strictly, so a checked key with the
// wrong JSON type fails.
func decodeManifest(raw []byte) (manifest, error) {
	kept, err := checkedKeys(raw)
	if err != nil {
		return manifest{}, err
	}
	var m manifest
...
// checkedKeys returns raw with only the checked keys. An extensions value that
// is not an object is ignored.
func checkedKeys(raw []byte) ([]byte, error) {
