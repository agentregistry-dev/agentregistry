...
func TranslateRegistryServer(
	...
	if useRemote { //nolint:nestif
		...
		headers, err := platformshared.ProcessHeaders(remote.Headers, nil)
		if err != nil {
			return nil, err
		}
		...
		args = platformshared.ProcessArguments(args, pkg.RuntimeArguments, nil)

		// Determine image and command based on registry type
		config, args, err := platformshared.GetRegistryConfig(pkg, args)
		if err != nil {
			return nil, err
		}

		// Process package arguments after the package identifier
		args = platformshared.ProcessArguments(args, pkg.PackageArguments, nil)

		// Process environment variables
		envVarsMap, err := platformshared.ProcessEnvironmentVariables(pkg.EnvironmentVariables, envOverrides)
		if err != nil {
			return nil, err
		}
		envVars := platformshared.EnvMapToStringSlice(envVarsMap)
