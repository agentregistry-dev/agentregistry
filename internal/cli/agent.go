...
func publishAgent(cfg *publishAgentCfg) error {
	...
	// Parse environment variables from docker-compose file if it exists (generated by init)
	runtimeHint, packageArgs, envVars, err := parseParamsFromDockerCompose(cfg)
	if err != nil {
		return fmt.Errorf("failed to parse environment variables from docker-compose: %w", err)
	}
	...
	fmt.Println("arctl run agent " + jsn.Name)
...
func parseParamsFromDockerCompose(cfg *publishAgentCfg) (string, []model.Argument, []model.KeyValueInput, error) {

	agentName := getAgentNameFromManifest(cfg.ProjectDir)

	dockerComposeFile := filepath.Join(cfg.ProjectDir, "docker-compose.yaml")
	if _, err := os.Stat(dockerComposeFile); os.IsNotExist(err) {
		return "", nil, nil, nil
	}

	raw, err := os.ReadFile(dockerComposeFile)
	if err != nil {
		return "", nil, nil, fmt.Errorf("failed to read docker-compose file: %w", err)
	}
	var dockerComposeCfg struct {
	...
	if err := yaml.Unmarshal(raw, &dockerComposeCfg); err != nil {
		return "", nil, nil, fmt.Errorf("failed to unmarshal docker-compose file: %w", err)
	}
	svc, ok := dockerComposeCfg.Services[agentName]
	if !ok {
		return "", nil, nil, fmt.Errorf("service %s not found in docker-compose file", agentName)
	}
	...
	return runtimeHint, args, envVars, nil
}
