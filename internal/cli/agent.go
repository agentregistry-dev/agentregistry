...
func publishAgent(cfg *publishAgentCfg) error {
	...
	envVars, err := parseEnvVarsFromDockerCompose(cfg)
	if err != nil {
...
func parseEnvVarsFromDockerCompose(cfg *publishAgentCfg) ([]model.KeyValueInput, error) {
	...
	if !ok {
		return nil, fmt.Errorf("service %s not found in docker-compose file", agentName)
	}
