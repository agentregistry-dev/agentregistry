// Package chat is the interactive A2A chat TUI launched after `arctl run`
// brings an agent up. It speaks the A2A streaming protocol via trpc-a2a-go
// and renders user/agent turns + tool calls/results in a bubbletea model.
//
// Resurrected from internal/cli/agent/tui (Phase 12 deletion); package
// renamed from `tui` to `chat` because it now lives under declarative/.
package chat
...
import (
	...
	"github.com/agentregistry-dev/agentregistry/internal/cli/commands/chat/theme"
)
...
// LaunchA2A is a convenience wrapper that creates an A2A client against
// agentURL and starts the chat TUI. Used by `arctl run` after the agent
...
func LaunchA2A(ctx context.Context, agentName, agentURL string, verbose bool) error {
	...
	return RunChat(agentName, sessionID, sendFn, verbose)
}
