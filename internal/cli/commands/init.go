...
import (
	...
	"github.com/agentregistry-dev/agentregistry/internal/cli/commands/mcpresolve"
...
init and add an MCP_SERVERS_CONFIG entry, e.g.:
  ...
  arctl init agent myagent --local-mcp ../my-mcp`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := writeDeclarativeAgentYAML(projectDir, name, image, initDescription, initMCPs); err != nil {
...
func writeDeclarativeAgentYAML(projectDir, name, image, description string, mcps []string) error {
	desc := description
	if desc == "" {
