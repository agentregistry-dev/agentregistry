...
func runStatus() error {
	...
	daemonStatus := checkDaemonHealth()
	if daemonStatus.healthy {
		printer.PrintSuccess(fmt.Sprintf("Daemon is running (%s)", daemonStatus.url))
	} else {
		printer.PrintError(fmt.Sprintf("Daemon is not reachable (%s): %s", daemonStatus.url, daemonStatus.err))
		return nil
	}
...
func checkDaemonHealth() daemonHealthResult {
	...
	if err != nil {
		return daemonHealthResult{healthy: false, url: url, err: err.Error()}
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return daemonHealthResult{healthy: false, url: url, err: fmt.Sprintf("HTTP %d", resp.StatusCode)}
	}

	return daemonHealthResult{healthy: true, url: url}
}
