...
var PushCmd = &cobra.Command{
	...
	Short: "Wrap and push a Claude Skill as a Docker image without publishing",
	Long: `Wrap a Claude Skill in a Docker image and push it to both Docker registry and agent registry.
...
func init() {
	...
	PushCmd.Flags().BoolVar(&pushPushFlag, "push", false, "Automatically push to Docker and agent registries")
...
func runPush(cmd *cobra.Command, args []string) error {
	...
	for _, skill := range skills {
		...
		skillJson, err := buildSkillDockerImageForPush(skill)
		if err != nil {
	...
	if !pushDryRunFlag {
		printer.PrintSuccess("Skill pushed successfully")
	}

	return nil
}

func buildSkillDockerImageForPush(skillPath string) (*models.SkillJSON, error) {
