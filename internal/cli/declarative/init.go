...
func writeDeclarativeMCPYAML(projectDir, name, image, description string, port int, transport string, launchDefaults *frameworks.MCPLaunch) error {
	desc := description
	if desc == "" {
	...
	// Stdio packages need a non-empty Command + Args at deploy time so the
	// runtime knows what to exec inside the container. HTTP transport leaves
	// launch nil — the runtime configures the container's listen address from
	// the transport block itself. A framework without launch defaults leaves
	// launch nil for stdio too; the user can hand-edit mcp.yaml.
	var launchBlock *v1alpha1.MCPPackageLaunch
	if transport == "stdio" && launchDefaults != nil {
		launchBlock = &v1alpha1.MCPPackageLaunch{
			Command: launchDefaults.Command,
			Args:    launchDefaults.ToMCPArguments(),
		}
	}
