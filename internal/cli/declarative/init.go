...
Picks a framework + language interactively (or via --framework / --language).`,
  ...
  arctl init mcp my-stdio --framework fastmcp --language python --transport stdio`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := writeDeclarativeMCPYAML(projectDir, name, image, initDescription, initPort, initTransport); err != nil {
				return err
			}
...
func writeDeclarativeMCPYAML(projectDir, name, image, description string, port int, transport string) error {
	desc := description
	if desc == "" {
