...
- **Unauthenticated by default.** When enabled, the compatibility routes bypass configured credential authentication and ignore presented tokens. Without additional authorization hooks, the catalogue is flat and unfiltered across namespaces. The feature is disabled by default; enable it only where anonymous read access is acceptable.
- **Authorization hooks remain available.** Forks and embedding applications can use the existing per-kind `ListFilter` and `Authorize` hooks to constrain which servers anonymous callers can read. First-party authentication is future work.
